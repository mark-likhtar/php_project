<?php
    ...
    // $query_comment = "INSERT INTO `child_comment`(`comment_id`, `user_id`, `child_comment_text`) VALUES ('$url', '$user_id','$comment_text')";
    ...
    if($result_name)
    {
        ...
        if($result_comment){
            ...
            for ($j = 0 ; $j < $rows_comment ; ++$j)
            {
                ...
                echo "</div>";
                echo "</div>";
                $query_child_comment = "SELECT child_comment.`child_comment_text`, user.`name`, child_comment.`date`, user.`admin` FROM `child_comment` inner join `user` on child_comment.`user_id` = user.`user_id` WHERE child_comment.`comment_id`='$comment' order by child_comment.`date`";
                ...
                if($result_child_comment){
                    ...
                    if($_SESSION['login'] == !''){
                        echo "<div class='forum_child_comment_form'>";
