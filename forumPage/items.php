<?php
    ...
    $query_name = "SELECT forum.`name`, forum.`forum_id`, user.`name`, CAST(forum.`date` as date) FROM `forum` INNER JOIN `user` ON forum.`user_id` = user.`user_id` ORDER BY forum.`date`";
    $result_name = mysqli_query($link, $query_name) or die("Ошибка " . mysqli_error($link));
    
    $query_name_language = "SELECT forum.`name`, forum.`forum_id`, user.`name`, CAST(forum.`date` as date) FROM `forum` INNER JOIN `user` ON forum.`user_id` = user.`user_id` WHERE `language`= '$url' order by forum.`date`";
