...
$query_name = "SELECT post.`name`, post.`content`, COUNT(post_likes.`like_id`), post.`post_id` FROM `post` LEFT OUTER JOIN `post_likes` ON post.`post_id` = post_likes.`post_id` WHERE post.`type` = 'lesson' GROUP BY post.`post_id` ORDER BY COUNT(post_likes.`like_id`) DESC";
